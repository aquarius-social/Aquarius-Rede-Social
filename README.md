...
Devem passar **222 testes** com `OK`. (Em ambiente Windows onde `python` é o
...
- **`ci.yml`** — roda os 222 testes + type-check TS a cada push/PR.
- **`ingestao.yml`** — agenda a ingestão 2×/dia (06h/18h BRT) + disparo manual.
  ...
  0001–0010 no Supabase é pré-requisito (uma vez, via CLI ou painel).
