      ...
      {opcoes.map((o) => {
        ...
        return (
          <Pressable key={o.l} onPress={() => onSel(o.v)} style={[st.chip, { backgroundColor: s ? cor.navy : cor.white, borderColor: s ? cor.navy : cor.border }]}>
            <Text style={{ color: s ? cor.white : cor.navy, fontWeight: '600', fontSize: 11.5 }}>{o.l}</Text>
          </Pressable>
        );
      })}
    </ScrollView>
...
const st = StyleSheet.create({
  ...
  emBreveTit: { fontSize: 18, fontWeight: '800', color: cor.navy },
  ...
  contador: { fontSize: 11.5, color: cor.mutedSoft, fontWeight: '700', letterSpacing: 0.4, marginBottom: 6, paddingLeft: 2, textTransform: 'uppercase' },
