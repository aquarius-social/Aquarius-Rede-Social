    ...
    <View style={{ flex: 1, backgroundColor: cor.surface }}>
      ...
      <View style={st.header}>
        ...
        <Pressable style={st.bell} hitSlop={6} onPress={() => {}}>
          <Icon name="bell" size={18} color={cor.navy} />
        </Pressable>
...
function TabBtn({ label, on, onPress }: { label: string; on: boolean; onPress: () => void }) {
  return (
    <Pressable onPress={onPress} style={st.tab}>
      <Text style={[st.tabTxt, on && { color: cor.navy, fontFamily: fonte.xb }]}>{label}</Text>
    ...
    <View style={st.card}>
      ...
      <View style={st.acoes}>
        <Pressable style={st.acaoBtn} onPress={() => Linking.openURL(URL_EMENDAS_CONSULTA)}>
          <Icon name="doc" size={14} color={cor.navy} />
...
const st = StyleSheet.create({
  ...
  bell: { width: 36, height: 36, borderRadius: 9999, borderWidth: 1, borderColor: cor.border, backgroundColor: cor.white, alignItems: 'center', justifyContent: 'center' },
  ...
  tabUnder: { position: 'absolute', left: 13, right: 13, bottom: 0, height: 2.5, borderRadius: 9999, backgroundColor: cor.navy },
  ...
  card: { backgroundColor: cor.white, borderRadius: raio.cardGrande, borderWidth: 1, borderColor: cor.border, padding: 14, marginBottom: 12 },
  ...
  autorNome: { fontFamily: fonte.b, fontSize: 13.5, color: cor.navy },
  autorRole: { fontFamily: fonte.b, fontSize: 10, color: cor.mutedSoft, letterSpacing: 0.6, textTransform: 'uppercase' },
  headline: { fontFamily: fonte.xb, fontSize: 16.5, color: cor.navy, letterSpacing: -0.3, lineHeight: 22 },
  ...
  payloadValor: { fontFamily: fonte.xb, fontSize: 19, color: cor.navy, letterSpacing: -0.4 },
  ...
  acaoBtn: { flex: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 6, paddingVertical: 10, borderRadius: 9999, borderWidth: 1, borderColor: cor.border, backgroundColor: cor.white },
  acaoPrometeus: { backgroundColor: cor.navy, borderColor: cor.navy },
  acaoTxt: { fontFamily: fonte.b, fontSize: 12, color: cor.navy },
